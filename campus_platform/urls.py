"""
URL configuration for campus_platform project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/4.2/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import path
from django.urls import include
from django.conf import settings
from django.conf.urls.static import static

urlpatterns = [
    path('admin/', admin.site.urls),
    path('',include(('users.urls','users'),namespace='users')),
    path('homepage/',include(('homepage.urls','homepage'),namespace='homepage')),
    path('forum/',include(('forum.urls','forum'),namespace='forum')),
    path('activities/',include(('activities.urls','activities'),namespace='activities')),
    path('materials/',include(('materials.urls','materials'),namespace='materials')),
    path('confess/',include(('confess.urls','confess'),namespace='confess')),
    path('lostfound/',include(('lostfound.urls','lostfound'),namespace='lostfound')),
    # path('market/',include(('market.urls','market'),namespace='market')),  # 二手市场功能暂时隐藏
    path('news/',include(('news.urls','news'),namespace='news')),
] + static(settings.MEDIA_URL,document_root=settings.MEDIA_ROOT)
